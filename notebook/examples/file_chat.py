...
def load_environment():
    ...
    # Get the project root directory (one level up from notebook/examples)
    ...
    if not os.getenv('OPENAI_API_KEY'):
        ...
        print(f"Please add your OpenAI API key to {env_path} as OPENAI_API_KEY=your-key-here")
        sys.exit(1)
...
def main():
    ...
    # Default data directory relative to the script location
    default_data_dir = str(Path(__file__).parent / 'data-sample')
    
    input_group = parser.add_mutually_exclusive_group(required=False)
    input_group.add_argument(
        '--input-dir',
        default=default_data_dir,
        help=f'Directory containing documents to process (default: {default_data_dir})'
    )
    input_group.add_argument(
        '--files',
        nargs='+',
    ...
    parser.add_argument(
        '--model',
        default='gpt-4o-mini',
        help='Specific model to use (default: gpt-4o-mini)'
    )
    ...
    try:
        ...
        if args.load_existing:
            video_path, index_path = load_existing_memory(Path(args.load_existing))
        else:
                    # args.input_dir already has the default value set in the argument parser
            ...
            if args.input_dir:
                ...
                files = collect_files_from_directory(str(input_dir), set(args.extensions))
                if not files:
                    print(f"No supported files found in {input_dir}")
                    print("Supported extensions:", 
                          ', '.join(['.txt', '.md', '.pdf', '.doc', '.docx', '.rtf', '.epub', '.html', '.htm']))
                    print(f"Current working directory: {os.getcwd()}")
                    print(f"Looking in: {input_dir.absolute()}")
                    return 1
                    
                memory_name = args.memory_name or generate_memory_name(input_dir)
                ...
                video_path, index_path = create_memory_from_files(
                    files, output_dir, memory_name, **config
                )
                
            elif args.files:
                files = [str(Path(f).absolute()) for f in args.files]
                memory_name = args.memory_name or generate_memory_name(files)
